    ...
    def get_price_info(self, city_name) :
        ...
        for i in range(1, total_page) :
            ...
            for house_content in house_contents :
                ...
                # 打印单条房价信息
                print("\t===> name: %s, price: %s 元/平米, total: %s" %(name.encode("utf-8"), price.encode("utf-8"), total.encode("utf-8")))

                # 格式化单条房价信息，并添加到list中
                price_fmt_str = self.format_price_info(name.encode("utf-8"), price.encode("utf-8"), total.encode("utf-8"))
